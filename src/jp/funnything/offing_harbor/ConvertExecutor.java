...
public class ConvertExecutor {
    public void execute(Project project, VirtualFile file, ConvertConfig config) {
        ...
        Notifications.Bus.notify(new Notification("OffingHarbor", "FindViewById", "哈哈，代码已复制~", NotificationType.INFORMATION), project);
    }
    ...
    private String generateJavaCode(List<AndroidViewInfo> infos, Tree viewNameTree, ConvertConfig config) {
        ...
        for (AndroidViewInfo info : infos) {
            ...
            if (config.who == ConvertConfig.Who.$) {
                methodJavaCode.append(String.format("    %s = $(R.id.%s);" + NL, symbol, info.id));
            } else if (config.who == ConvertConfig.Who.VIEW) {
                methodJavaCode.append(String.format("    %s = view.findViewById(R.id.%s);" + NL, symbol, info.id));
            } else if (config.who == ConvertConfig.Who.ITEM_VIEW) {
                methodJavaCode.append(String.format("    %s = itemView.findViewById(R.id.%s);" + NL, symbol, info.id));
            } else {
